...
> Загрузка файлов изображений не входит в требуемый стек (только Firestore + Auth), поэтому фото объявлений задаются внешними ссылками (поле «Ссылки на фото» в форме публикации).

## Быстрый старт

1. Создайте проект в [Firebase Console](https://console.firebase.google.com/).
2. **Authentication** → Sign-in method → включите **Email/Password**.
3. **Firestore Database** → создайте базу (режим production, любой регион).
4. **Project settings → General → Your apps** → добавьте Web-приложение, скопируйте объект конфигурации.
5. Вставьте его в [js/firebase-config.js](js/firebase-config.js) вместо плейсхолдеров `YOUR_API_KEY` и т.д.
6. (Опционально) Разверните правила и индексы:
   ```bash
   npm install -g firebase-tools
   firebase login
   firebase use --add        # выберите ваш проект
   firebase deploy --only firestore:rules,firestore:indexes
   ```
   Индексы можно и не разворачивать заранее — Firestore при первом запросе, которому не хватает индекса, выведет в консоль браузера прямую ссылку на его создание.
7. Запустите локальный статический сервер (ES-модули не работают с `file://`):
   ```bash
   python3 -m http.server 8080
   # или: npx serve .
   ```
8. Откройте `http://localhost:8080`, зарегистрируйтесь.
9. Чтобы стать администратором: в Firestore Console откройте `users/<ваш uid>` и вручную поставьте `role: "admin"` (это единственный шаг, который нельзя сделать иначе без Cloud Functions — назначать роль дальше уже можно из самой админ-панели → вкладка «Пользователи»).
10. Зайдите в `admin.html` → вкладка «Статистика» → кнопка **«Заполнить демо-данными»**, чтобы за один клик получить 20 тестовых объявлений и проверить каталог, поиск, фильтры и пагинацию.
...
css/                          дизайн-система (styles.css) + стили отдельных страниц
